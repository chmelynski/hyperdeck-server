...
// LinkedList : { head : Node , tail : Node , listSize : 0 }
// Node : { prev : Node , next : Node , data : Breakpoint }


var LinkedList = (function(undefined) {
	
	function LinkedList() { this.head = null; this.tail = null; this.listSize = 0; };
	LinkedList.Node = function (data) { this.prev = null; this.next = null; this.data = data; };
	LinkedList.Node.prototype.toString = function () { return this.data.toString(); };
	LinkedList.prototype.isLinked = function (node) {
		return !((node && node.prev === null && node.next === null && this.tail !== node && this.head !== node) || this.isEmpty());
	};
	LinkedList.prototype.size = function () { return this.listSize; };
	LinkedList.prototype.isEmpty = function () { return this.listSize === 0; };
	LinkedList.prototype.first = function () { return this.head; };
	LinkedList.prototype.last = function () { return this.last; };
...
// the node types
var glue = function(width, stretch, shrink) { return { type : 'glue' , width : width , stretch : stretch , shrink : shrink }; };
var box = function(width, value) { return { type : 'box' , width : width , value : value }; };
var penalty = function(width, penalty, flagged) { return { type : 'penalty' , width : width , penalty : penalty , flagged : flagged }; };
...
// formatter functions calls linebreak extensively
// Typeset.formatter.center : function(text) => [ Node ]
// Typeset.formatter.justify : function(text) => [ Node ]
// Typeset.formatter.left : function(text) => [ Node ]
// there is of course some duplication in having a separate function for each textAlign
//   it is unclear whether this duplication can be cleanly abstracted
// there is an identical preamble, variables spaceStretch and spaceShrink, which are not even used in center and left
// also, where is right justification?
// and who cares?  what's the point of using Knuth-Plass if you don't even want perfectly crisp edges?  why not just use a simpler algo?
// which means, why do we even care for functions other than justify?
Typeset.formatter = function(measureText, options) {
	...
	var o = {};
	o.space = {};
	o.space.width = options && options.space.width || 3;
	o.space.stretch = options && options.space.stretch || 6;
	o.space.shrink = options && options.space.shrink || 9;
...
// is this even used?  aren't 3-6-9 hard coded in formatter anyway?
//Typeset.formatter.defaults = { space : { width : 3 , stretch : 6 , shrink : 9 } };

// this replaces Typeset.formatter.justify
function Justify(text, measureText, options) {
	...
	var spaceWidth = measureText(' ');
	var spaceStretch = (spaceWidth * o.space.width) / o.space.stretch;
	var spaceShrink = (spaceWidth * o.space.width) / o.space.shrink;
	...
	words.forEach(function(word, index, array) {
		...
		if (hyphenated.length > 1 && word.length > 4)
		{
			hyphenated.forEach(function(part, partIndex, partArray) {
				...
				if (partIndex !== partArray.length - 1) { nodes.push(penalty(hyphenWidth, hyphenPenalty, 1)); }
			});
		}
		else
		{
...
function Linebreak(nodes, lines, settings) {
	...
	var options = {};
	options.demerits = {};
	options.demerits.line = settings && settings.demerits && settings.demerits.line || 10;
	options.demerits.flagged = settings && settings.demerits && settings.demerits.flagged || 100;
	options.demerits.fitness = settings && settings.demerits && settings.demerits.fitness || 3000;
	options.tolerance = settings && settings.tolerance || 2;
	...
	var breaks = DetermineBreaks(activeNodes);
	return breaks;
};
	...
	while (active !== null)
	{
		...
		for (var fitnessClass = 0; fitnessClass < candidates.length; fitnessClass += 1)
		{
			...
			if (candidate.demerits < Infinity)
			{
				...
				var newNode = new LinkedList.Node(bp);
...
function DetermineBreaks(activeNodes) {
	...
	var tmp = { data : { demerits : Infinity } };
	var breaks = [];
	...
	activeNodes.forEach(function (node) { if (node.data.demerits < tmp.data.demerits) { tmp = node; } });
...
function DrawText(ctx, text, x, y, measureText, lineWidths, linePitch) {
	var nodes = Justify(text, measureText); // Node : { type : 'box' | 'glue' | 'penalty' , width : 0 , value : "foo" } // etc. each type has different fields
	//console.log(nodes);
	var breaks = Linebreak(nodes, lineWidths); // break : { position : 0 , ratio : 0 }
	console.log(breaks);
	var placements = PlaceWords(nodes, breaks, lineWidths); // Placement : { text : "foo" , x : 0 , line : 0 }
	//console.log(placements);
	placements.forEach(function(p) { ctx.fillText(p.text, x + p.x, y + p.line * linePitch); });
}
...
// Hypher.en = { leftmin : 2 , rightmin : 2 , patterns : { 3 : <string> , 4 : <string> , ... } }
// Hypher.en is fed to the constructor Hypher(language)
// Hypher.hyphenateText('foobar') => 'foo-bar'
// Hypher.hyphenate('foobar') => [ 'foo' , 'bar' ]
// Hypher.createTrie({ 3 : <string> , 4 : <string> , ... }) => Hypher.TrieNode (= { points : [ number ] })
...
function Hypher(language) {
	...
	this.trie = this.createTrie(language['patterns']); // Hypher.TrieNode
	this.leftMin = language['leftmin']; // number
	this.rightMin = language['rightmin']; // number
	this.exceptions = {}; // !Object.<string, !Array.<string>>
	...
	for (size in patternObject)
	{
		if (patternObject.hasOwnProperty(size))
		{
			...
			patterns.forEach(function(pattern)
			{
				...
				var points = pattern.split(/\D/);
				var t = tree;
				
				chars.forEach(function(c)
				{
					var codePoint = c.charCodeAt(0);
					if (!t[codePoint]) { t[codePoint] = {}; }
					t = t[codePoint];
				});
				
				t._points = points.map(function(p) { return p || 0; });
			});
		}
	}
	return tree;
...
Hypher.prototype.hyphenateText = function(str, minLength) {
	...
	// Regexp("\b", "g") splits on word boundaries, compound separators and ZWNJ so we don't need any special cases for those characters.
...
Hypher.prototype.hyphenate = function(word) {
	...
	if (this.exceptions.hasOwnProperty(word)) { return this.exceptions[word]; }
	...
	for (i = 0; i < wordLength; i += 1) { points[i] = 0; characterPoints[i] = characters[i].charCodeAt(0); }
