...
export class PDF extends General {
	...
	// so these call the super?
	set lineWidth(value: number)      { this.PushCommand(value.toString() + ' w'); }
	set fillStyle(value: string)      { this.PushCommand(this.ConvertColorToPdf(value) + ' rg'); }
	set strokeStyle(value: string)    { this.PushCommand(this.ConvertColorToPdf(value) + ' RG'); }
	set lineDashOffset(value: number) { this.PushCommand('[ ' + this._lineDashArray.join(' ') + ' ] ' + this._lineDashOffset.toString() + ' d'); }
	set lineJoin(value: string)       { this.PushCommand({miter:'0',round:'1',bevel:'2'}[value] + ' j'); }
	set lineCap(value: string)        { this.PushCommand({butt:'0',round:'1',square:'2'}[value] + ' J'); }
	set miterLimit(value: number)     { this.PushCommand(value.toString() + ' M'); }
