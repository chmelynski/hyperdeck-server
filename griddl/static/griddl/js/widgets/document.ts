...
export class Document {
	...
	_add(): void {
		...
		var div = $('<div class="docParams"></div>').appendTo(doc._div);
		
		var unit = $(MakeSelector(['in','cm','mm','pt'], doc.unit)).on('change', function() {
		...
		var pageNumberHoriAlign = MakeSelector(['left','center','right','alternateLeftRight','alternateRightLeft'], doc.pageNumberHoriAlign).css('width', '10em').on('change', function() { doc.pageNumberHoriAlign = this.value; });
		var pageNumberVertAlign = MakeSelector(['top','center','bottom'], doc.pageNumberVertAlign).css('width', '10em').on('change', function() { doc.pageNumberVertAlign = this.value; });
		...
		$('<button>Generate</button>').addClass('btn btn-default').css('margin-right', '0.5em').on('click', function() { doc._generate(); }).appendTo(div);
		$('<button>Export</button>').addClass('btn btn-default').on('click', function() { doc._exportToPdf(); }).appendTo(div);
		...
		$('<hr>').appendTo(doc._div);
		
		var sectionsDiv = $('<div class="sectionsDiv"></div>').appendTo(doc._div);
		...
		for (var i = 0; i < this.sectionControlArray.length; i++)
		{
			...
			sectionControl.listElement = this.sectionControlList._add(sectionControl);
			sectionControl._add();
		}
		...
		var contentsDiv = $('<div class="contentsDiv"></div>').appendTo(doc._div);
		...
		for (var i = 0; i < this.contentControlArray.length; i++)
		{
			...
			contentControl.listElement = this.contentControlList._add(contentControl);
			contentControl._add();
		}
	...
	_generate(): void {
		...
		var sectionControlArray = this.sectionControlList._enumerate();
		this.sections = [];
		
		for (var i = 0; i < sectionControlArray.length; i++)
		{
			var sectionParams = sectionControlArray[i];
			var selector = sectionParams.selector;
			var compnames = selector.split(',').map(function(x) { return x.trim(); });
			
			var htmls = [];
			
			for (var k = 0; k < compnames.length; k++)
			{
				var html = Hyperdeck.Get(compnames[k]); // so how do we figure out whether it's a html or a md?  possibly will need to pass in an option
				htmls.push(html);
			}
			
			var section = Parse(htmls);
			section.doc = this;
	...
	_newPage(width: number, height: number): CanvasRenderingContext2D {
		
		var canvas = $('<canvas></canvas>').attr('width', width).attr('height', height).css('border', '1px solid gray').appendTo(this._div);
	...
	_exportToPdf(): void {
		...
		//};
		//
		//if (MathJax) { MathJax.Hub.Queue(RenderPdf); } else { RenderPdf(); }
	}
	_draw(): void {
		...
		{
			this.sections[i]._draw();
		}
		
		// draw page numbers
		if (this.doPageNumbering)
		{
			var n = 1;
			for (var i = 0; i < this.sections.length; i++)
			{
				...
				var wd = ((section.params.orientation == 'portrait') ? this.pageWidth : this.pageWidth) * this.cubitsPerUnit;
				var hg = ((section.params.orientation == 'portrait') ? this.pageHeight : this.pageHeight) * this.cubitsPerUnit;
				
				for (var k = 0; k < section.nPages; k++)
				{
					var hAlign = this.pageNumberHoriAlign;
					var vAlign = ((this.pageNumberVertAlign == 'center') ? 'middle' : this.pageNumberVertAlign);
					...
					ctx.font = this.pageNumberFont;
					ctx.fillStyle = this.pageNumberColor;
					...
					if (n == 1 && !this.firstPage) { n++; continue; }
					
					var x = xs[hAlign] + xPolarity[hAlign] * this.pageNumberHoriOffset;
					var y = ys[vAlign] + yPolarity[vAlign] * this.pageNumberVertOffset;
	...
	_write(): any {
		...
		return {
			...
			sectionParams: this.sectionControlList._enumerate.map(function(x) { return x.write(); }),
			contentParams: this.contentControlList._enumerate.map(function(x) { return x.write(); })
		};
	}
	_afterAllLoaded(): void {
		var doc = this;
		doc._generate();
	}
}
class Section {
	...
	_draw(): void {
		...
		var wd = ((this.params.orientation == 'portrait') ? this.doc.pageWidth : this.doc.pageHeight) * this.doc.cubitsPerUnit;
		var hg = ((this.params.orientation == 'portrait') ? this.doc.pageHeight : this.doc.pageWidth) * this.doc.cubitsPerUnit;
		...
		{
			if (this.contents[i].params.pageIndex > nPages)
			{
				nPages = this.contents[i].params.pageIndex;
			}
		}
		...
		for (var i = 0; i < this.contents.length; i++)
		{
			pages[this.contents[i].params.pageIndex - 1].contents.push(this.contents[i]);
		}
		
		if (debug) { console.log(`pages needed by widgets: ${nPages}`); }
		
		var columnWidth = (wd - this.params.marginLeft - this.params.marginRight - this.params.interColumnMargin * (this.params.columns - 1)) / this.params.columns;
		...
		for (var i = 0; i < nPages; i++)
		{
			...
			for (var k = 0; k < this.params.columns; k++)
			{
				var lf = this.params.marginLeft + (columnWidth + this.params.interColumnMargin) * k;
				var rt = lf + columnWidth;
				var tp = i * hg + this.params.marginTop;
				var bt = (i + 1) * hg - this.params.marginBottom;
		...
		if (this.params.pitch < 0.01) { throw new Error('line height too small'); }
		...
		for (var i = 0; i < pages.length; i++)
		{
			...
			var bt = box.tp + this.params.pitch;
			...
			while (true)
			{
				if (bt > box.bt)
				{
					...
					for (var k = boxIndex; k < page.boxes.length; k++)
					{
						...
						{
							page.boxes[k].tp = bt - this.params.pitch;
						}
					}
				...
				else
				{
					...
					line.box.reconcile({lf : box.lf , bt : bt , wd : box.wd , hg : this.params.pitch });
					page.lines.push(line);
					lines.push(line);
				}
				
				bt += this.params.pitch;
			}
		}
		...
		var lineTexts: PlacedWord[][] = LinebreakJustify(lineWidths, this.words, this.wordMetrics, this.params.spaceWidth, this.params.minSpaceWidth);
		...
		if (lineTexts.length > lines.length)
		{
			...
			var tp = this.params.marginTop;
			var bt = tp + this.params.pitch;
			var pageBottom = hg - this.params.marginBottom;
			...
			for (var i = 0; i < excess; i++)
			{
				...
				var lf = this.params.marginLeft + (columnWidth + this.params.interColumnMargin) * currentColumn;
				var wd = columnWidth;
				var hg = this.params.pitch;
				...
				if (bt > pageBottom)
				{
					...
					if (currentColumn >= this.params.columns)
					{
						currentColumn = 0;
						tp = this.params.marginTop;
						bt = tp + this.params.pitch;
						pageBottom = hg - this.params.marginBottom;
	...
	_calculateWordMetrics(): void {
		
		//if (this.textStyle.style) { SetStyle(this.ctx, this.textStyle.style); }
...
class SectionParams {
	...
	_add(): void {
		...
		var orientation = $(MakeSelector(['portrait', 'landscape'], params.orientation)).on('change', function() {
			params.orientation = this.value;
		});
		
		var columns = $(MakeSelector(['1', '2', '3', '4'], params.columns.toString())).on('change', function() {
...
function MakeSelector(options: string[], selected: string): any {
...
function MakeColorControls(): any {
